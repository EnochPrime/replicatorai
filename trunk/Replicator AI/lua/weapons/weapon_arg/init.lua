...
function SWEP:PrimaryAttack(fast)
	...
	local delay = 0;
	if(self.AttackMode == 1 and ammo >= 30 and not fast) then
...
function SWEP:DoShoot()
	...
	if(ammo >= 20) then
		...
		local trgt = ents.FindInSphere(pos + (100*normal),75);
		local num = 0;
		for k,v in pairs(trgt) do
			local class = v:GetClass();
			if(class == "npc_rep_n" or class == "npc_rep_l" or class == "npc_rep_h") then
				v:arg(p,self.AttackMode);
				num = k;
			end
		end
		if (num ~= 0) then
			trgt[num]:RecentUpdate(self.AttackMode);
		end
	end
end
